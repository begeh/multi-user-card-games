...
const { Turn } = require('./game-logic-classes.js.js');
...
// THIS IS ASYNC. BE WARNED

// TO DO: write an insert query function to write turn results!


const getGameData = async function (game_id) {
  const queryString = `
  SELECT
  game_id,
  ...
  const res = await pool.query(queryString, [`${game_id}`]);
  return res.rows;
}
