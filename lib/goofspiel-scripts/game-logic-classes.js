//GAME LOGIC CLASSES:
// the primary class here is TURN. turn takes in a position in the result array of a getGameData function
// and creates a game board complete with methods to advance the game.
//
//
// TO INIT: pass in the result of getGameData at the desired turn index! The class handles deconstruction
// by itself.
//
// TO USE:
// run on the SERVER SIDE. Instantiate it with getTurnData at each new turn.
// pass data into it, get data out, and pass that data back to the client.
// the clients need only send and recieve raw data. the client should not need to handle anything else other than rendering.
//
// Turn.dealer.deal() to deal a card, and returns the value of that card! Do this at the start of a turn.
//
// Turn.Player.play(card) to play a card. if that card is already in play, it will rescind the card!
// Ideally, do this when a card is clicked.
//
// Turn.Player.isReady() to toggle ready state. Do this on ready button click
//
// Turn.resolveTurn() to resolve the turn and return an object containing all values to be written to the database.
// It will do nothing and return FALSE if both players' ready states are not truthy!
//
